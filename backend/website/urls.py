...
from .views.shareViews import get_shares, delete_share, create_share, view_share, create_share_zip_model, share_view_stream, share_view_thumbnail, share_view_subtitle, \
    share_get_subtitles, check_share_password, get_share_visits, get_visit_events, share_stream_zip_files
...
urlpatterns = [
    ...
    path("files/<signed_file_id>/zip-entry/stream", ["GET"], stream_zip_entry, name="stream zip entry"),
    ...
    path("files/<file_id>/moments", ["GET"], get_moments, name="get a moment"),
    path("files/<file_id>/moments", ["POST"], add_moment_view, name="add a moment"),
    ...
    path('user/can-upload/<folder_id>', ['GET'], can_upload, name="check if user is allowed to upload"),
    path("user/settings", ['PUT'], update_settings, name="update settings"),

    path("user/discord-settings", ['GET'], get_discord_settings_view, name="get discord settings"),
    path("user/discord-settings", ['PATCH'], update_attachment_name_view, name="update upload destination"),
    path("user/discord-settings", ['DELETE'], reset_discord_settings_view, name="reset discord settings"),
    path("user/discord-settings/setup", ['POST'], discord_settings_start_view, name="do auto setup"),
    path("user/discord-settings/create-webhooks", ['POST'], create_channel_and_webhook_view, name="add a channel and webhooks"),
    path("user/discord-settings/webhooks/<webhook_id>", ['DELETE'], delete_webhook_view, name="delete a webhook"),
    path("user/discord-settings/bots", ['POST'], add_bot_view, name="add a bot"),
    path("user/discord-settings/bots/<bot_id>", ['DELETE'], delete_bot_view, name="delete a bot"),
    path("user/discord-settings/credentials/<credential_id>/enable", ['POST'], reenable_credential_view, name="reenable a credential"),
    ...
    path("shares/<token>/zip/<zip_token>/stream", ['GET'], share_stream_zip_files, name="share zip stream files"),
