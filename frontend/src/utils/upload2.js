...
import { chunkSize, requestType, uploadType } from "@/utils/constants.js"
...
async function convertUploadInput(uploadInput, type) {
   /**
...
export async function preUploadRequest(request) {
   if (request.type === requestType.multiAttachment) {
      //for (let attachment of request.attachments) {}
      //check if folder created, if not create it.
      //iterate tru request.attachments, add create {data} add to list
      //create file on the backend, add fileObj returned from backend to the request
   }
   if (request.type === requestType.chunked) {

      //check if attachment.fileObj already created.
      // If not check if folder created if not create it
      //then create file
      //add fileObj returned from backend to the request
   }
}
